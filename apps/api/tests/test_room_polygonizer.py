...
class TestRoomPolygonizer:
    ...
    def test_rectangle_with_door_gap_still_one_room(self):
        ...
        mask = _draw_walls((400, 500), [
            (50, 50, 450, 50),
            (450, 50, 450, 350),
            (50, 350, 235, 350),     # нижняя — левая часть
            (265, 350, 450, 350),    # нижняя — правая часть (gap 30px ≈ 0.6м)
            (50, 50, 50, 350),
        ])
        walls = [
            _wall("w0", 50, 50, 450, 50),
            _wall("w1", 450, 50, 450, 350),
            _wall("w2", 50, 350, 235, 350),
            _wall("w3", 265, 350, 450, 350),
    ...
    def test_two_rooms_with_inner_wall(self):
        ...
        mask = _draw_walls((400, 600), [
            ...
            (50, 350, 550, 350),
            (50, 50, 50, 350),
            # Внутренняя стена с разрывом 30px (≈ 0.6м межкомнатная дверь)
            (300, 50, 300, 185),
            (300, 215, 300, 350),
        ])
        walls = [
            ...
            _wall("w3", 50, 50, 50, 350),
            _wall("w4", 300, 50, 300, 185, WallType.inner),
            _wall("w5", 300, 215, 300, 350, WallType.inner),
        ]
        rooms, _ = polygonize_rooms(mask, walls, px_per_meter=50.0)
        # Door gap 30px должен закрыться, внутренняя стена остаётся
